...
class _SignInScreenState extends State<SignInScreen> with SingleTickerProviderStateMixin{
  ...
  late Offset position = Offset(Get.width / 2 - ballSize / 2, 0);
  int step = 0;
  ...
  void initState() {
    ...
    _controller.addListener(() {
     ...
     if(step == 0){
       double newDy = ((Get.height / 2) - (ballSize / 2)) / 100;
        double newDx = ((Get.width / 2) - (ballSize / 2)) / 100;
        print('newDx: $newDx');
        print('newDy: $newDy');
        print('position.dx: ${position.dx}');
        print('position.dy: ${position.dy}');
       setState(() {
         position = Offset((Get.width / 2 - ballSize / 2) - (newDx * _controller.value), newDy * _controller.value);
       });
      }else if(step == 1) {
      print('step 1');
      double newDy = ((Get.height / 2) - (ballSize / 2)) / 100;
      ...
      double textX = (Get.width / 2 - ballSize / 2) - (newDx * _controller.value);
      double testY = newDy * _controller.value;
      print('textX: $textX');
      setState(() {
        position = Offset(textX, ((Get.height) - (ballSize)) - ( newDy * _controller.value));
      });
     }else if(step ==2){
       print('step 2');
        double newDy = ((Get.height / 2) - (ballSize / 2)) / 100;
        ...
        print('textX: $textX');
        setState(() {
          position = Offset((Get.width / 2 - ballSize / 2) + (newDx * _controller.value), (Get.height - ballSize) - newDy * _controller.value);
        });
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          Positioned(
            left: ((Get.width ) - (ballSize )) - position.dx,
            top: ((Get.height ) - (ballSize )) - position.dy,
            child: Container(
              width: ballSize,
              height: ballSize,
              decoration: BoxDecoration(
